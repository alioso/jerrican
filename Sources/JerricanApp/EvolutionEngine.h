...
// Timbre, Groove, Wander, Dissonance, Busy, Sustain) to wander on their own
// over time, scaled by two independent global controls: Amount (0 = the
// macros stay exactly where the user set them — update() is a no-op
// regardless of Speed; 1 = new targets are picked often) and Speed (how
// quickly the current value glides toward whichever target was last picked
// — independent of how often a new one is chosen). Each macro can also be
// individually opted out via setXEnabled(false), so some of a voice's
// controls can stay under manual control while others evolve.
...
// 8 macros now (grown from the original 6 to add Busy/Sustain for Bass, and
// more voices expected to grow their own bespoke macros in future rounds),
// spelling each one out by hand would make this file grow linearly forever.
...
public:
    enum class Macro { Volume, PitchCenter, Timbre, Groove, Wander, Dissonance, Busy, Sustain };
