...
// A single shared, sample-accurate tempo clock, driving Bass's
// BassGroovePattern off the same grid. Ticked once per sample from the
// audio callback (same per-sample call convention GrainCloud uses) rather
// than batched per-block, so there's no need to track multiple boundary
// crossings within one call — simpler, and avoids any real-time
// allocation. Ported from Marmite's PatternClock.h, unchanged — this class
// is already voice-count-agnostic.
class PatternClock {
public:
