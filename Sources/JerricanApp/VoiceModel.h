...
// between them).
//
// busy_/sustain_/attack_ are Bass-only, cleanliness_ is Ambient-only (see
// JerricanProcessor.h's kInitialVoices) — idle/unused for whichever voices
// don't consume them, same tier as how Grain::Character and GrainCloud's
// duration-range constants are already per-voice-archetype fields that
// only some voices make active use of.
class VoiceModel {
public:
    ...
    VoiceModel(std::string name, bool enabled, float volume, float pitchRangeLow,
               ...
               float sustain = 0.5f, float cleanliness = 0.5f, float attack = 0.5f)
        : name_(std::move(name)),
          enabled_(enabled),
          ...
          cleanliness_(clamp01(cleanliness)),
          attack_(clamp01(attack)) {}
    ...
    float getCleanliness() const { return cleanliness_.load(std::memory_order_relaxed); }
    ...
    void setCleanliness(float cleanliness) {
        cleanliness_.store(clamp01(cleanliness), std::memory_order_relaxed);
    }
    void setAttack(float attack) { attack_.store(clamp01(attack), std::memory_order_relaxed); }
    ...
    std::string getSummary() const {
        return name_ + " | enabled=" + (isEnabled() ? "true" : "false") +
               ...
               " | cleanliness=" + std::to_string(getCleanliness()) +
