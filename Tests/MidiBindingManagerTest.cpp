...
int main() {
    ...
    // the first — one knob/pad should never silently drive two things.
    {
        ...
        manager.armLearn(MidiTarget::VoiceTimbre);
        manager.handleEvent(event);
        assert(!manager.getBinding(MidiTarget::VoiceVolume).has_value());
        assert(manager.getBinding(MidiTarget::VoiceTimbre).has_value());

        const auto resolved = manager.handleEvent(event);
        assert(resolved.has_value());
        assert(*resolved == MidiTarget::VoiceTimbre);
    }
    ...
    { assert(kAllMidiTargets.size() == 36); }
